...
namespace AlumnoEjemplos.LOS_IMPROVISADOS.EfectosPosProcesado
{
	...
	/// </summary>
	public class PosProcesoDistorsion : APosProcesado
	{
		
		float time;

		public PosProcesoDistorsion(Mapa mapa):base(mapa)
		{
			this.mapa = mapa;
		}
		...
		public override void init()
		{
            ...
            effect = TgcShaders.loadEffect(GuiController.Instance.AlumnoEjemplosDir + "\\LOS_IMPROVISADOS\\Shaders\\EfectoDistorsion.fx");

            effect.Technique = "DistorsionTechnique";
...
//		}
		...
		public override void render(float elapsedTime){
			...
			device.Clear(ClearFlags.Target | ClearFlags.ZBuffer, new SharpDX.ColorBGRA(0, 0, 0, 255), 1.0f, 0);
			effect.SetValue("time", time);
			
			foreach(TgcMesh mesh in meshes){
				mesh.Effect = effect;
				mesh.Technique = "DistorsionTechnique";
				mesh.render();
			}
		...
		public override void drawSceneToRenderTarget(Device d3dDevice)
		{
            ...
            {
				m.Effect = effect;
				m.Technique = "DistorsionTechnique";
                m.render();
            }
		}
